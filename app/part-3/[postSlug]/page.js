...
const chap = {
  MR: "Market Risk Measurement & Management",
  CR: "Credit Risk Measurement & Management",
  OR: "Operational Risk & Resilience",
  LR: "Liquidity & Treasury Risk Measurement & Management",
  IM: "Risk Management & Investment Management",
    ...
    <div className="relative group-hover:flex z-20 shadow-2xl ">
      <div className="absolute left-[20px]  top-2 rounded-xl p-2 md:p-4 bg-white  border-gray-300 shadow-xl w-[200px] sm:w-[250px] md:w-[350px] hidden group-hover:block z-20 before:content-[''] before:absolute before:top-[-10px] after:shadow-2xl before:left-[50%] before:transform before:translate-x-[-50%] before:border-[12px]  before:border-transparent before:border-b-gray-300 before:border-t-0 before:border-l-0 before:border-r-0">
        ...
        <div className="pt-1">
          ...
          <p className="text-[14px] mt-2 text-center md:text-[18px] lg:text-[20px] font-semibold tracking-wide leading-[15px] sm:leading-6" style={{ color: "Black" }}>{authorName}</p>
          <p className="text-[12px] text-black text-center md:text-[14px] lg:text-[16px] font-[500] my-1 sm:my-[6px]">{authorInfo.qualification}</p>
          <p className="text-center text-black font-[400] text-[12px] md:text-[14px] lg:text-[17px]">{authorInfo.description}</p>
        </div>
      </div>
    </div>
  );
...
export async function generateMetadata({ params }) {
  try {
    ...
    return {
      title: `${title.name || title.number} - MidhaFin(MF)`,
      ...
      openGraph: {
        ...
        url: `https://frm.midhafin.com/part-2/${params.postSlug}`, // Corrected URL
        locale: "en_IN",
...
export default async function Post({ params }) {
  ...
  let jsonSchema;
  try {
    jsonSchema = seoData.schema.raw.replace(/https:\/\/cmsfrm.midhafin.com\/\d{4}\/\d{2}\/\d{2}\/([^/#]+)(#[^/]*)?/g, "https://frm.midhafin.com/part-2/$1");
  } catch (error) {
  ...
  //   const pdfArray = String.split(/>|&gt;/);
  //   const pdfObjects = pdfArray.reduce((acc, pdf, index) => {
  //     if (pdf.trim().length > 0) {
  //       const [question, ...answerParts] = pdf.trim().split("?");
  //       const answer = answerParts.join("?").trim();
  //       acc.push({ id: index, text: question.trim(), data: answer });
  //     }
  //     return acc;
  ...
  function parsePDF(pdfString) {
    const pdfArray = pdfString.split(/>|&gt;/);
    return pdfArray.reduce((acc, pdf, index) => {
      if (pdf.trim().length > 0) {
        const [title, url] = pdf.split("?");
        acc.push({ id: index, title: title.trim(), url: url.trim() });
      }
      return acc;
    }, []);
  }

  const generatePDFSchema = (pdfArray) => {
    const pdfSchema = {
      "@context": "https://schema.org",
      "@type": "ItemList",
      itemListElement: pdfArray.map((pdf, index) => ({
        "@type": "ListItem",
        position: index + 1,
        name: pdf.title,
        url: pdf.url,
    ...
    return <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(pdfSchema, null, 2) }} />;
  };
  ...
  const pdfData = postData.pdf.pdfs ? parsePDF(postData.pdf.pdfs) : [];
  ...
  const baseUrl = `https://frm.midhafin.com`; // Corrected base URL
    ...
    <>
     <style>{`
      ...
      .prose ul ul li::marker {
        list-style-type: circle; /* Circular appearance for sub-bullets */
        color: #202124; /* Dark color for sub-bullets */
      }
    `}</style>
      <LatexScriptLoader />

      <div>
        <div className="bg-[#6A1C1A] text-white text-left px-[20px] md:px-[50px] py-8">
          ...
          <div className="text-[14px] md:text-[18px] font-[400] max-w-[1000px] items-center mx-auto flex flex-wrap justify-between">
        ...
        <div id="chapter" className="gap-[30px] max-w-[1400px] mx-auto px-5 md:px-[50px]">
          <BlogMenuBar video={videoLink} pdfs={pdfData} chapter={category} slug={params.postSlug} />
          <div className="flex justify-between lg:gap-8">
            ...
            <div className="max-w-[1100px] w-[100%] pt-5">
              ...
              <div className="prose text-[.88rem] sm:text-[1.05rem] lg:text-[1.25rem] w-full flex-shrink-0" style={{fontFamily: 'Merriweather, Lato, Georgia, serif'}} dangerouslySetInnerHTML={{ __html: content }}></div>
            </div>
          </div>
        </div>
        <hr className="mt-5" />
...
export const revalidate = 0; // Revalidate every 10 mins
