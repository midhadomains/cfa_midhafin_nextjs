  ...
  try {
    ...
    const tagNames = data.tags?.nodes?.map((tag) => tag.name) || [];    return {
      ...
      openGraph: {
        ...
        url: `https://frm.midhafin.com/${params.postSlug}`,
        locale: "en_IN",
...
export default async function Post({ params }) {
  ...
  let jsonSchema;
  try {
    jsonSchema = seoData.schema.raw.replace(/https:\/\/cmsfrm.midhafin.com\/\d{4}\/\d{2}\/\d{2}\/([^/#]+)(#[^/]*)?/g, "https://frm.midhafin.com/$1");
  } catch (error) {
  ...
  const baseUrl = "https://frm.midhafin.com";
  return (
    <>
    <LatexScriptLoader/>
