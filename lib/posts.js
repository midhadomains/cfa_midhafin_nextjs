...
export async function getPostList(endCursor = null, taxonomy = null) {
  let condition = `after: "${endCursor}", first: 9, where: {orderby: {field: DATE, order: DESC}, categoryNotIn:["15","16"]}`;

  if (taxonomy) {
    condition = `after: "${endCursor}", first: 9, where: {orderby: {field: DATE, order: DESC}, ${taxonomy.key}: "${taxonomy.value}", categoryNotIn:["15","16"]}`;
  }
...
export async function getKeywords(slug) {
  const query = {
    query: `query getSinglePost {
            post(id: "${slug}", idType: SLUG) {
                tags(first: 100) {
...
export async function getPostSlugs() {
  const query = {
    query: `query getPostSlugs {
              posts(first: 10000, where: {categoryNotIn:["15", "16"]}) {
              nodes {
                slug
              }
            }
...
export async function getCategorySlugs() {
  ...
  categories(where: {exclude:["576","121","1","15","16","443","76","394"]}) {
    nodes {
      slug
      name
