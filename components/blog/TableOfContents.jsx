...
//             </button>
//             <div className={` table-of-contents bg-white shadow-lg z-20 w-[230px] md:w-[260px] fixed xl:sticky mg:left-2 right-1 top-[65px] rounded-t-xl border-l border-r border-[#BE4E1E42] transition-all ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} style={{ transform: `translateX(${isVisible ? '0%' : '100%'})` }}>
...
//                 <ul className='px-2 z-20 max-h-[400px] overflow-y-scroll'>
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      for (let i = 0; i < sectionOffsets.length - 1; i++) {
        ...
        if (scrollPosition >= currentSection.offset && scrollPosition < nextSection.offset && activeSection !== currentSection.id) {
          setActiveSection(currentSection.id);
          break;
        }
      }
                    ...
                    <React.Fragment key={section.id}>
                        <li className="py-2 text-[13px] md:text-[16px]">
                            <Link
                                href={`#${section.id}`}
                                onClick={() => scrollToSection(section.id)}
                                className={`hover:text-[#BE4E1E] ${activeSection === section.id ? 'text-[#BE4E1E] font-[600] active-section' : ''}`}
                            >
                                {section.title}
                            </Link>
                        </li>
